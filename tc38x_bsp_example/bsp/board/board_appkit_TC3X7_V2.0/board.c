...
void bsp_board_wdg_Disable(void)
{
  ...
  do {  /* Make sure that flags in STATUS register are cleared */
  ...
  for(i = 0; i < wdt_cmd_size; ++i) {
    ...
    while(!module_qspi->STATUS.B.TXF) {  /* wait until transfer is complete */
      ;
    }
    while(!module_qspi->STATUS.B.RXF) {  /* wait for receive is finished */
      ;
    }

    module_qspi->FLAGSCLEAR.U = 1 << 10;  /* clear RX flag */
    (void) module_qspi->RXEXIT.U;  /* read and discard value */
  }
...
#endif /* #if defined (UNT_ZCU_TC387QP_V1_0) */
